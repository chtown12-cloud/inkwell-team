...
/* Every client now says "still here" every 25s, so a player who has genuinely
   gone can be dropped in minutes rather than a quarter of an hour. It stays
   well clear of the heartbeat because phones throttle background timers, and
   anyone swept out by mistake is put straight back by the rejoin on return. */
 ...
 */
function applyAction(room, body, nowMs) {
  // The caller supplies the time because these rules run on whichever device
  // is acting. Two phones with clocks a few minutes apart would otherwise
  // write timestamps that the idle sweep reads as ancient, and evict players
  // who are sitting right there. Clients pass a server-corrected clock.
  ...
  // Stamp the caller BEFORE sweeping. Doing it the other way round meant a
  // player coming back after a pause — a phone unlocked, a tab refocused —
  // was evicted by their own keep-alive, and had to rejoin to reappear.
  ...
  switch (action) {
    ...
    case 'leave': {
      // Someone closing the game should stop appearing on the shared screen at
      // once, rather than lingering until the idle sweep notices.
      if (!me) break;
      room.players = room.players.filter((p) => p.id !== playerId);
      for (const objId of Object.keys(room.seats)) {
        room.seats[objId] = room.seats[objId].filter((p) => p !== playerId);
        if (!room.seats[objId].length) delete room.seats[objId];
      }
      break;
    }
