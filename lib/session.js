...
const MAX_NAME = 20;
const MAX_SOLVED = 40;
const IDLE_MS = 90 * 1000;      // seat auto-release
...
function applyAction(room, body) {
  ...
  switch (action) {
    ...
    case 'sit': {
      ...
      if (room.solved.includes(body.objId)) break; // nothing to sit on
      // Seats are PRESENCE, not a gate: a puzzle never refuses a player and one
      // person may solve alone. The only bound is a safety cap so the array
      // cannot grow past the player limit.
...
module.exports = {
  makeCode, newRoom, applyAction, reap, seatCount,
